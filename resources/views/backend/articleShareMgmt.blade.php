...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary title">Article Share List</h6>
    </div>
    <div class="card-body">
        <div style="margin-bottom: 1.5em;">
            <button type="button" onclick="redirect('articleShareMgmtEdit?type=add')" class="btn btn-info">新增</button>
        </div>
...
<script>
    function delList(obj, id){
        ...
        gMyJs.doAjax(gBasePath + 'ajaxArticleShareMgmt', {'data': gMyJs.jwtEncode({'id': id, 'ajaxType': 'delList', 'delType': 'articleShare'})},
    ...
    function reSetPhoto(id, imgId, photoType){
        gMyJs.doAjax(gBasePath + 'ajaxArticleShareMgmt', {'data': gMyJs.jwtEncode({'id': id, 'photoType': photoType, 'ajaxType': 'reSetPhoto'})},
            function(data, resData){
                if (resData.success) {
        ...
        gDataTable = $('#dataTable').DataTable({
            ...
            "aoColumns": [
                ...
                { "sTitle": "管理", "mData": "id", "mRender": function(data, type, row){
                        return '<button type="button" onclick="redirect(\'articleShareMgmtEdit?type=edit&id=' + data + '\')" class="btn btn-info">管理</button>';
                    },
            ...
            "ajax": {
                ...
                "url": gBasePath + 'ajaxArticleShareMgmt',
                "type": "POST",
                "data": function (d) {
                    var data = {
                        'ajaxType': 'getArticleShareList',
                        'cid': $('#category_id').val(),
                    };
