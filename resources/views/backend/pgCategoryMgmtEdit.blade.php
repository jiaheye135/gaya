...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary title">簡介</h6>
    </div>
    <div class="card-body">
        <div class="item-group">
            <input id="introduction" name="introduction" type="text" class="form-control bg-light" placeholder="">
            <div class="my-error_msg"></div>
        </div>    
    </div>
</div>
<div class="row">
    <div class="col-lg-6">
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary title">*排序<span class="my-sub_title">(由大到小, 請輸入數字或小數)</span></h6>
            </div>
            <div class="card-body">
                <div class="item-group">
                    <input id="sort" name="sort" type="text" class="form-control bg-light" placeholder="" value=''>
                    <div class="my-error_msg"></div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function save(btnObj, submitList, groupClassName){
        ...
        formData.append('data', gMyJs.jwtEncode({'data': data, 'type': '{{ $type }}', 'id': {{ $dbId }}, 'ajaxType': 'ajaxPgCategory'}));
