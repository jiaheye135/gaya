...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary title">分類圖片<span class="my-sub_title">(建議尺寸:500*400)</span></h6>
    </div>
    <div class="card-body">
        <div class="item-group">
            <label class="btn btn-info" style="margin-bottom: 0;">
                <input id="img" name="img" class="" style="display: none;" type="file" accept=".jpg,.jpeg,.png" data-type="singlefile">
...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary title">*分類名稱</h6>
    </div>
    <div class="card-body">
        <div class="item-group">
...
<div class="row">
    <div class="col-lg-6">
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary title">*排序<span class="my-sub_title">(由大到小)</span></h6>
            </div>
            <div class="card-body">
                <div class="item-group">
...
<script>
    ...
    function save(btnObj, submitList, groupClassName){
        ...
        formData.append('data', gMyJs.jwtEncode({'data': data, 'type': '{{ $type }}', 'id': {{ $dbId }}, 'ajaxType': 'ajaxSsCategory'}));
        ...
        gMyJs.doAjax('{{ $basePath }}ajaxStudentShareMgmt', formData, 
            function(data, resData){
                setLoadingBtn(btnObj, 'close');

                if (resData.success) {
                    alert('儲存成功');
                    redirect('ssCategoryMgmt');
                } else {
                    alert('儲存失敗');
                }
            },
