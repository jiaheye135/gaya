...
<div class="card shadow mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary title">Photo Gallery Category List</h6>
    </div>
    <div class="card-body">
        <div style="margin-bottom: 1.5em;">
            <button type="button" onclick="redirect('pgCategoryMgmtEdit?type=add')" class="btn btn-info">新增</button>
        </div>
...
<script>
    function delList(obj, id){
        ...
        gMyJs.doAjax(gBasePath + 'ajaxPhotoGalleryMgmt', {'data': gMyJs.jwtEncode({'id': id, 'ajaxType': 'delList'})},
    ...
    function add(categoryId){
        var param = 'data=' + gMyJs.jwtEncode({'categoryId': categoryId});
    ...
    $(function(){
        gDataTable = $('#dataTable').DataTable({
            ...
            // ],
            "aoColumns": [
                { "sTitle": "分類名稱", "mData": "name" },
                { "sTitle": "排序", "mData": "sort" },
                { "sTitle": "狀態", "mData": "state", "mRender": function(data, type, row){
                    return (data == 1) ? '啟用' : '不啟用';
                } },
                { "sTitle": "上傳相片", "mData": "id", "mRender": function(data, type, row){
                        return '<button type="button" onclick="add(' + data + ')" class="btn btn-success">上傳相片</button>';
                    },
                    "orderable": false, "width": "7em"
                },
                { "sTitle": "管理", "mData": "id", "mRender": function(data, type, row){
                        return '<button type="button" onclick="redirect(\'pgCategoryMgmtEdit?type=edit&id=' + data + '\')" class="btn btn-info">管理</button>';
                    },
                    "orderable": false, "width": "7em"
                },
            ...
            "ajax": {
                ...
                "url": gBasePath + 'ajaxPhotoGalleryMgmt',
                "type": "POST",
                "data": function (d) {
                    var data = {
                        'ajaxType': 'getList',
